...
export async function POST(req) {
  try {
    const { to, via } = await req.json();
    if (!to || !via) return resSend({success: false, error: "Missing fields" },400);
    ...
    //   });
    // }
    console.log(`✔️ ---- Sent OTP ${code} to ${to}`);
    return resSend({ ok: true, success: true, message: `OTP sent to ${to}` })

  } catch (e) {
    console.log("OTP send error:", e);
    return resSend({success:false, error: "Failed to send OTP" }, 500 )
  }
}
